  ...
  static async create(data: CreateAnnouncementInput) {
    const { rows } = await db.query(
      `INSERT INTO announcements (id, title, body, date, priority, posted_by, expires_date, status)
       ...
       RETURNING *`,
      [data.id, data.title, data.body, data.date, data.priority, data.posted_by, data.expires_date]
    );
    return rows[0];
  }
  ...
  static async update(id: string, data: UpdateAnnouncementInput) {
    const { rows } = await db.query(
      `UPDATE announcements
       ...
       WHERE id = $8
       RETURNING *`,
      [data.title, data.body, data.date, data.priority, data.posted_by, data.expires_date, data.status, id]
    );
    return rows[0] || null;
  }
